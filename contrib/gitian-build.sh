...
VERSION=
commit=false
url=https://github.com/c2fcoin/c2fcoin
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/c2fcoin/c2fcoin
...
then
    ...
    git clone https://github.com/c2fcoin/gitian.sigs.git
    git clone https://github.com/c2fcoin/c2fcoin-detached-sigs.git
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit c2fcoin=${COMMIT} --url c2fcoin=${url} ../c2fcoin/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../c2fcoin/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/c2fcoin-*.tar.gz build/out/src/c2fcoin-*.tar.gz ../c2fcoin-binaries/${VERSION}
	popd
