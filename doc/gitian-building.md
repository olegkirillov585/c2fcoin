...
*Setup instructions for a gitian build of C2FCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the C2FCoin
...
- [Building C2FCoin](#building-c2fcoin)
...
Clone the git repositories for c2fcoin and gitian and then checkout the c2fcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/c2fcoin-crypto/c2fcoin.git
cd c2fcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the c2fcoin repository
...
To build C2FCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the c2fcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/c2fcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/c2fcoin-crypto/c2fcoin
...
For example:
```bash
URL=https://github.com/crowning-/c2fcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit c2fcoin=${COMMIT} --url c2fcoin=${URL} ../c2fcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit c2fcoin=${COMMIT} --url c2fcoin=${URL} ../c2fcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit c2fcoin=${COMMIT} --url c2fcoin=${URL} ../c2fcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/c2fcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/c2fcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/c2fcoin-build.assert
```
...
[c2fcoin/gitian.sigs](https://github.com/c2fcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
