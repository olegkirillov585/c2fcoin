                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zC2F ought to be enough for anybody." - Bill Gates, 2017
    ui->zC2FpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzC2FSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzC2F->setEnabled(false);
        ui->pushButtonMintzC2F->setToolTip(tr("zC2F is currently disabled due to maintenance."));

        ui->pushButtonSpendzC2F->setEnabled(false);
        ui->pushButtonSpendzC2F->setToolTip(tr("zC2F is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zC2FpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzC2F_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC2F is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zC2F, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zC2F...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zC2F in ") +
...
void PrivacyDialog::on_pushButtonSpendzC2F_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zC2F is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zC2F
        sendzC2F();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zC2F
    sendzC2F();
}

void PrivacyDialog::on_pushButtonZC2FControl_clicked()
{
    ZC2FControlDialog* ZC2FControl = new ZC2FControlDialog(this);
    ZC2FControl->setModel(walletModel);
    ZC2FControl->exec();
}

void PrivacyDialog::setZC2FControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzC2FSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzC2F()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid C2FCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zC2FpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zC2F is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " C2F </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zC2F</b>";
    ...
    // use mints from zC2F selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZC2FControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZC2FControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zC2F was spent successfully update the addressbook with the label
    ...
    // Clear zC2F selector in case it was used
    ZC2FControlDialog::listSelectedMints.clear();
    ui->labelzC2FSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zC2F Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zC2F </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zC2F "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zC2F "));
    ui->labelzC2FAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zC2F </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zC2F </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzC2FSyncStatus->setVisible(fShow);
}
