...
#include "ui_zc2fcontroldialog.h"
...
std::list<std::string> ZC2FControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZC2FControlDialog::listMints;

ZC2FControlDialog::ZC2FControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZC2FControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZC2FControlDialog::~ZC2FControlDialog()
{
    delete ui;
}

void ZC2FControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZC2FControlDialog::updateList()
{
...
void ZC2FControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZC2FControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelzC2F_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZC2FControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZC2FControlDialog::GetSelectedMints()
{
...
void ZC2FControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
