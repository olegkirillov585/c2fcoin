...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("C2FCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("C2FCoinMiner : ProcessNewBlock, block not accepted");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    CValidationState state;
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("C2FCOINMiner : ProcessBlock, block not accepted");
...
{
    LogPrintf("C2FCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("c2fcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running C2FCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
