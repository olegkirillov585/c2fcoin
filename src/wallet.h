...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 C2F at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zC2F send
enum ZerocoinSpendStatus {
    ZC2F_SPEND_OKAY = 0,                            // No error
    ZC2F_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZC2F_WALLET_LOCKED = 2,                         // Wallet was locked
    ZC2F_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZC2F_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZC2F_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZC2F_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZC2F_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZC2F_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZC2F_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZC2F_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZC2F_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZC2F_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZC2F_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZC2F_SPENT_USED_ZC2F = 14,                      // Coin has already been spend
    ZC2F_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZC2FAutoBackups(bool fEnabled)
    {
    ...
    /** zC2F reset */
    boost::signals2::signal<void()> NotifyzC2FReset;
};
